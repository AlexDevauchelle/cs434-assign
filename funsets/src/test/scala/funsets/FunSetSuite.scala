...
class FunSetSuite extends FunSuite {
  ...
  test("filter returns elements that satisfy the predicate") {
    new TestSets {
  ...
  test("exists returns true when at least one element satisfies the predicate") {
    new TestSets {
      val s = union(s1, s2)
  ...
  test("map transforms the set according to the given function") {
    new TestSets {
      val s = union(s1, s2)
      val mappedSet = map(s, x => x * 2)
      assert(contains(mappedSet, 2), "Map 1")
      assert(contains(mappedSet, 4), "Map 2")
      assert(!contains(mappedSet, 1), "Map 3")
    }
  }
}
