...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      // Run the provided function with the created CancellationToken
      val execution = f(cancellationTokenSource.cancellationToken)

      // When the Subscription is unsubscribed, cancel the CancellationToken
      val subscription = new Subscription {
        def unsubscribe(): Unit = {
          cancellationTokenSource.unsubscribe()
        }
      }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete {
        case Success(result) => p.success(cont(Future.successful(result)))
