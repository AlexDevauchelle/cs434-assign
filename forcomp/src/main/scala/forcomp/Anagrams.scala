...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(c => c).map { case (char, occurrences) => (char, occurrences.length) }.toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    (y foldLeft x.toMap) { case (occMap, (char, count)) =>
      val newCount = occMap(char) - count
      if (newCount <= 0) occMap - char
      else occMap updated(char, newCount)
    }.toList.sorted
